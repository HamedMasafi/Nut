...
QStringList AbstractSqlGenerator::diffRelation(TableModel *oldTable, TableModel *newTable)
{
    ...
    QStringList columnSql;
    for (QString &fieldName: relations) {
...
QString AbstractSqlGenerator::updateRecord(Table *t, QString tableName)
{
    ...
    for (QString f: t->changedProperties())
        if (f != key)
                                        ...
                                        const ConditionalPhrase &where)
{
    ...
    for (PhraseData *d: assigments.data) {
...
{
    for (TableModel *&m: _database->model())
        command = command
...
{
    QString ret = QString();
    for (const PhraseData *d: ph.data) {
        if (!ret.isEmpty())
            ret.append(QStringLiteral(", "));
